...
const getSEDashboardApplicationSummary = async (req, res) => {
  try {
    ...
    if (officer.missingDivision) return res.status(400).json({ error: "SE user has no division assigned" });
    ...
    console.error("getSEDashboardApplicationSummary error:", error);
    return res.status(500).json({ error: "Server Error" });
  }
};

const getSEDashboardApplications = async (req, res) => {
  try {
    const userId = String(req.query.userId || "").trim();
    ...
    console.error("getSEDashboardApplications error:", error);
    return res.status(500).json({ error: "Server Error" });
  }
};
